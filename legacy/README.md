> **Retired 2026-08-22.** Everything below is the previous shipped game — a
> daily driving-hours route puzzle — plus a never-shipped canvas map engine
> prototype that was being built as its replacement. Both are superseded by
> the core-loop redesign; see `/README.md` for the current direction. Kept
> here, still buildable and playable, for reference and possible reuse —
> nothing under `core-loop/` imports from here.
>
...
web/map/     the never-shipped canvas map engine prototype (see docs/MAP-SPEC.md)
docs/SPEC.md the one-page spec for the game above
```

The generation pipeline that fills `web/` (`scripts/00`–`03`, `05`–`11` at
the repo root) still runs and still writes here — see `/README.md` for what
